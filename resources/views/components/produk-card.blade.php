<div class="w-[380px] bg-zinc-700 p-4 flex h-44 rounded-3xl shadow gap-4">
    <div class="flex flex-col justify-between">
        ...
        <div class="gap-4 text-center justify-between items-center flex">
            <div @click="$store.productCart.removeFromCart({{ $product->id }})"
            ...
            <h1 class="text-white font-bold text-xl" x-text="$store.productCart.getItemQuantity({{ $product->id }})"></h1>
            <div @click="$store.productCart.addToCart({{ $product->id }}, '{{ $product->name }}', {{ $product->price }}, {{ $product->point }})"
